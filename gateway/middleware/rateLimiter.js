...
const rateLimiter = ({ getKey, getLimitConfig }) => {
    return async (req, res, next) => {
        try {
            ...
            try {
                ...
                tokens = tokens ? parseInt(tokens) : limitConfig.tokens;
                lastRefill = lastRefill ? parseInt(lastRefill) : now;
                ...
                const memKey = `${limitConfig.name}:${identity}`;
